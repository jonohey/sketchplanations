...
1. **Audit** — `npm run tag-cleanup:audit`  
   ...
   Also reports **broken tag links** (sketches still pointing at deleted/archived tag documents). CI runs `npm run check:broken-tags` on every PR — fix orphans in the Prismic UI (remove or re-link tags on those sketches).

2. **Approve** — Copy rows you want into [`tag-merge-plan.csv`](tag-merge-plan.csv) (committed).  
...
7. **Archive** — Re-run audit; archive unused tag documents manually in Prismic UI.
