...
const calculateHorizontalPercentage = (
	bounds,
	threshold = 0,
	root = window,
) => {
...
const useScrollPercentage = (options = {}) => {
	const [internalRef, inView, entry] = useInView(options);
	const [percentage, setPercentage] = useState(0);
	const [ref, setRef] = useState(null);

	useEffect(() => {
		setRef(internalRef || document.body.querySelector("div"));
	}, [internalRef]);

	const target = entry?.target || ref;
	...
	return [setRef, percentage, entry];
};
